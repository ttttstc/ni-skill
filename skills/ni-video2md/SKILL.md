...
name: ni-video2md
description: |
  将抖音、X、YouTube、哔哩哔哩、小红书等公开视频 URL 或分享文案转成本地 Whisper 生成的 Markdown 文字稿。用户说“视频转文字”“视频转 md”“提取字幕/文字稿”时触发；优先本地转录，缺少 ffmpeg、whisper.cpp、模型、yt-dlp 或浏览器工具时先下载或安装。只输出 .md，不生成 SRT；标题和文件名统一为“全文一句话概括-作者”；交付后可按用户指定路径归档。
---
...
- 输入可以是单独的 URL，也可以是包含 URL 的完整分享文案；从文案中提取第一个 `http(s)` URL。支持抖音、X、YouTube、哔哩哔哩和小红书单个视频。
- 先检查本地依赖。缺少 `ffmpeg`、`whisper-cli`、Whisper 模型或 `yt-dlp` 时，先通过脚本下载/安装；抖音页面仍按需使用 Playwright/Chromium。找到或安装可执行文件后，把其目录加入当前进程 `PATH`，Windows 同步写入当前用户 `PATH`；不能因为依赖缺失直接切换到云端转录。
...
- Markdown 必须保留稳定来源 URL、捕获时间、语言、模型和 `local whisper.cpp` 标识；不要把带签名的临时媒体 URL 或小红书短期访问令牌写入 Markdown 或打印出来。
...
`${SKILL_DIR}` 是本 `SKILL.md` 所在目录。Windows 首次运行会在 `NI_VIDEO2MD_HOME`（未设置时为 `%LOCALAPPDATA%\\ni-video2md`）缓存便携版 ffmpeg、Whisper.cpp、`yt-dlp` 和模型；安装或发现 ffmpeg、Whisper.cpp、`yt-dlp` 后会自动把可执行文件目录加入当前进程 `PATH`，并持久化到当前用户 `PATH`。抖音才会复用已安装的 Chrome/Edge，没有可用浏览器时才下载 Playwright Chromium。已经打开的终端或 agent 需要重启后才能读取新的用户 `PATH`。脚本只下载公开依赖和视频，不需要 OpenAI API key。
...
| `NI_VIDEO2MD_JS_RUNTIME` | 指定 yt-dlp 使用的 JavaScript 运行时，如 `node` 或 `deno` |
| `NI_VIDEO2MD_BROWSER` / `BROWSER_PATH` | 指定 Chrome、Edge 或 Chromium 可执行文件 |

## 运行边界

1. 提取 URL。X、YouTube、哔哩哔哩和小红书通过 `yt-dlp` 读取公开元数据并下载单个视频；抖音使用无头浏览器捕获公开媒体流。`yt-dlp` 忽略用户配置，不读取 Cookie，不下载播放列表。小红书短期访问令牌只在当前下载过程使用，输出时会移除查询参数。
...
`yt-dlp` 的站点适配会随平台页面变化；X、YouTube、哔哩哔哩和小红书遇到验证码、登录墙、地区限制、页面变化或没有公开视频格式时，必须报告访问/兼容性失败，不能绕过限制或伪造文字稿。抖音仍未把旧版页面数据解析器或 `dyt` 作为必需依赖。
...
脚本成功时只报告 Markdown 路径；skill 随后交付该 Markdown，并询问是否归档。依赖下载失败、yt-dlp 无法读取或下载公开媒体、浏览器无法打开、没有媒体流、ffmpeg 转换失败或 Whisper 没有产出文字时，停止并给出下一步；不要降级到线上转录，也不要把部分结果冒充完整结果。

## 验收

- 输入为 URL 或分享文案时，能识别并处理抖音、X、YouTube、哔哩哔哩和小红书公开视频链接。
- 依赖缺失时先下载/安装，缓存后可复用；设置环境变量时优先使用用户指定路径。
- 找到或下载 ffmpeg、whisper-cli、`yt-dlp` 后，当前进程可以直接通过 `PATH` 找到它们；Windows 新开的终端或 agent 可以读取持久化的用户 `PATH`。
- 转录过程没有云端 Whisper/API 调用，且不会输出 API key、Cookie 或签名媒体 URL。
- 四个 yt-dlp 平台下载只处理单个公开视频，忽略本地 yt-dlp 配置；验证码、登录墙、付费内容和页面兼容性问题明确失败，不绕过访问控制。
...
- 失败场景明确可区分为依赖、yt-dlp 网络/媒体、浏览器、音频转换或转录失败。
