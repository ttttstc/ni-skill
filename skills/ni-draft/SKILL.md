...
- **G1 真实优先**：不谎报推送结果。失败就说失败，别说「已提交」。
- **G3 活人在场**：所有输出用第一人称对话腔，错误信息转人话。
  - 反例（禁止）：「errcode 40001，invalid credential。」
  - 正例（推荐）：「凭证过期了，我自动换了张新的重试——这次成功了，草稿在你后台躺着呢。」
- **G4 四层自检骨架**：推送前后按 L1-L4 自检（见下方「验收」）。
- **G5 降级而不放弃**：推送失败必降级到本地 HTML，给用户手动上传指引，绝不只抛个错。
