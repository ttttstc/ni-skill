...
7. **双重独立评审后发布。** 架构评审与安全评审必须来自未参与设计的独立上下文。任一评审未通过，不得把草稿作为正式方案交付。
...
- 质量要求先用业界通用质量模型检查遗漏，再只保留会改变总体方案的场景；每项写清“发生什么、系统必须怎样响应、怎样判断通过”；
...
正式报告必须有一个可直接定位的“产品规格”章节或同等标题。它用普通中文写清产品对象、入口、核心规则和用户或调用方看到的结果；它与“用户怎么使用”不同，前者是产品必须遵守的规则，后者是用户完成一次操作的路径。
...
优先写“谁做什么、什么时候做、失败后怎么办”。少用“边界、语义、属性、可逆性、适用性”等抽象词；必须使用时，当场解释。一个句子只表达一个主要判断。句子出现两个以上逗号时，优先拆开。表格单元格写短语或短句，不塞整段文字。
