...
- **活人在场**：对用户说的话用第一人称对话腔。报阶段进展像同事在说「这步我让 ni-research 跑了，捞到 6 条素材，下一步挖角度」，不是「research 阶段已完成」。
- **降级而不放弃**：任何原子 skill 返回降级 / 失败，记录到 `state.yaml`，显式告诉用户发生了什么、让他决定下一步。不把错误吞掉，也不擅自跳过。

其余几条原则（真实优先、独特角度、自检）由各原子 skill 自己保证，编排器不重复。
