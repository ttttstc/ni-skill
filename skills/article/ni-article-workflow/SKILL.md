...
description: |
  泥巴猪的公众号创作套件的编排器。串联 ni-research → ni-insight → ni-writer → ni-formatter → ni-inspect → ni-draft 的完整管线。当用户说「从头写一篇」「走完整管线」「一键发」「帮我把这篇从调研到发布走一遍」「跑创作流程」「完整流程写文章」时触发。本 skill 只编排，不含业务——所有调研、挖角度、写作、排版、预检、推送都委托给原子 skill。不适用于只想做单步（直接调对应原子 skill）、与公众号创作无关的任务。
---

# ni-article-workflow — 创作管线编排器

> 这是泥巴猪的公众号创作套件的编排器。它把 6 个原子 skill 串成一条从选题到发布的流水线。
...
- **只编排不做业务**：看到要写 / 排版 / 推送，调对应 skill，不自己上手。
- **状态机必持久化**：每个阶段结束写回 `state.yaml`，断点续跑靠这个。
- **显式降级**：任何 skill 返回 degraded / failed，记录到 `state.yaml.degradations` 并告诉用户原因 + 选项。
- **用户决策可回退**：用户手改 `state.yaml.phase` 回退到任意阶段，workflow 不阻拦。
...
2. 用对话腔显式告诉用户：「这一步降级了，原因是 X，你要继续还是先处理一下？」
3. 等用户决定，不擅自跳过、不擅自重试。
...
- **insight 阶段不通过不进 writing。** ni-insight 没拿到用户选定角度返回 failed 时，不许硬进写作阶段（独特角度门槛由 workflow 在编排层兜底）。
- **降级必须显式告知用户，不静默处理。**
